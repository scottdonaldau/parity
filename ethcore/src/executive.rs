...
impl<'a, B: 'a + StateBackend> Executive<'a, B> {
	/// Basic constructor.
	pub fn new(state: &'a mut State<B>, info: &'a EnvInfo, engine: &'a Engine) -> Self {
		Executive {
			state: state,
			info: info,
			engine: engine,
	...
	pub fn from_parent(state: &'a mut State<B>, info: &'a EnvInfo, engine: &'a Engine, parent_depth: usize) -> Self {
		Executive {
			state: state,
			info: info,
			engine: engine,
